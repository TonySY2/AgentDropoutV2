...
def prepare_livecode_data(item: dict) -> dict:
    problem = item.get('question_content', '')
    starter_code = item.get('starter_code', '')
    task_id = item.get('question_id', item.get('id', 'Unknown'))
    ...
    valid_tests = [t for t in test_cases if 'input' in t and 'output' in t]
...
class LiveCodeExecutor:
    ...
    def execute(self, code: str, test_cases: List[Dict]) -> Tuple[bool, str, str]:
        if not code:
        ...
        try:
            ...
            result_str = f"Passed {passed_count}/{total_count} public cases"
...
async def reasoning(
    ...
    starter_code,
    team: SelectorGroupChat,
    decision_maker: FinalWriteCodeMBPP,
    ...
    if is_baseline_mode:
        ...
        await Console(team.run_stream(task=full_task))
    ...
    raw_answer = await decision_maker.run_decision(
        history_messages=history_messages, role_map=role_map, task=full_task
    )
...
def write_to_file(out_file, data_id, data):
    ...
    try:
        sorted_keys = sorted(exist_data.keys(), key=lambda x: str(x))
    except:
...
async def run_sample(data, out_file, team, decision_maker, role_map, supervisor, log_file_path):
    ...
    try:
        ...
        answer, scores, reflection_records = await reasoning(
            task, starter_code, team, decision_maker, role_map, supervisor
        )
        ...
        if answer:
            try:
                executor = LiveCodeExecutor(timeout=4)
                is_solved, execution_result, error = executor.execute(answer, test_cases)
            except Exception as e:
                is_solved = False
                execution_result = "Executor Error"
                error = str(e)
        else:
            execution_result = "No code extracted"
    ...
    try:
        ...
        with open(args.in_file, "r", encoding='utf-8') as f:
            ...
            for idx, line in enumerate(lines):
                ...
                res = prepare_livecode_data(item)
