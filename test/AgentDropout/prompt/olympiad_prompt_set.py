...
# Revised descriptions for OlympiadBench (Open-ended, Proofs, Tuples, Sets)
ROLE_DESCRIPTION = {
    "Math Solver": 
        ...
        "Solve the problem step by step with rigorous proofs. "
        "Address all cases. If finding all solutions, ensure no solution is missed and no extraneous solution is included. "
        "Use LaTeX for mathematical expressions. "
        "The last line of your output must contain the final result boxed in LaTeX. "
        "For sets of solutions, format as: \\boxed{(x_1, y_1), (x_2, y_2)}. "
        "For proofs, box the final conclusion statement.\n",
        
    "Mathematical Analyst":
        "You are a mathematical analyst. "
        "Analyze the problem structure. Identify the subfield (e.g., Diophantine Equations, Functional Equations) and appropriate strategies (e.g., Modular Arithmetic, Bounding, Induction, Pigeonhole Principle). "
        "Break down complex conditions into manageable logical steps. "
        "Critique the approach: Are there edge cases? Is the bounding tight enough? "
        "Use LaTeX for mathematical expressions. "
        "The last line of your output must contain the final result boxed in LaTeX.\n",
        
    "Programming Expert":
        "You are a computational mathematics expert. "
        "Olympiad problems often cannot be solved purely by code, but code can find patterns or verify small cases. "
        "1. Write Python code to search for solutions in a reasonable range (e.g., x, y < 100) to generate conjectures. "
        "2. Use SymPy for symbolic manipulation if algebraic expressions are messy. "
        "3. If the problem asks for a specific value, try to compute it. "
    ...
    "Inspector":
        ...
        "Check for logical fallacies: Circular reasoning, division by zero, assuming what needs to be proved. "
        "Check for completeness: Did the solver find *all* solutions? Did they prove *necessity* and *sufficiency*? "
        "If specific examples were found by the programmer, ensure the mathematical solution accounts for them. "
...
@PromptSetRegistry.register('olympiad')
class OlympiadPromptSet(PromptSet):
    @staticmethod
    def get_role():
        return next(roles)
    ...
    def get_decision_constraint():
        ...
        "If the answer is a set of values or tuples, list them all. "
    ...
    def get_file_analysis_prompt(query, file):
        return (
            ...
            "2. specific conditions (e.g., 'p must be an odd prime') and check if they apply to the current problem.\n"
        )
    ...
    def get_websearch_prompt(question, query):
        return (
            ...
            "Example: 'x^3+y^3=p(xy+p) solutions', 'Order of a modulo p'."
        )
